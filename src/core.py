...
def efficient_frontier(mean_returns: np.ndarray, cov_matrix: np.ndarray, n_portfolios: int=1000, seed: int | None=None) -> Tuple[np.ndarray, list]:
...
def plot_efficient_frontier(results: np.ndarray, output_path: Path, plot: bool=False):
    ...
    scatter = ax.scatter(results[:, 1], results[:, 0], c=results[:, 2], cmap='viridis', alpha=0.6, s=20, edgecolors='none')
...
def plot_alpha_beta(market_returns: np.ndarray, asset_returns: np.ndarray, alpha: float, beta: float, output_path: Path, plot: bool=False):
    ...
    if not plot:
        return
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.scatter(market_returns, asset_returns, alpha=0.4, s=20, color='#4A90A4', edgecolors='none')
