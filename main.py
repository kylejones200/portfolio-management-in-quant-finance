...
def main():
    ...
    asset_returns = config["simulation"]["alpha_beta"][
        "asset_beta"
    ] * market_returns + np.random.normal(
        0,
        config["simulation"]["alpha_beta"]["asset_noise_std"],
        size=config["simulation"]["alpha_beta"]["n_periods"],
    )
    alpha, beta = calculate_alpha_beta(
        asset_returns, market_returns, config["simulation"]["alpha_beta"]["risk_free"]
    )
